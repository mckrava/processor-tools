...
class TypeormDatabase extends BaseDatabase {
    constructor(options) {
        ...
        this.disableAutoTxCommit = false;
        this.disableAutoHeightUpdate = false;
        this.saveBatchSize = 1000;
    ...
    async runTransaction(from, to, cb) {
        ...
        }, this.cacheStorage, this.saveBatchSize, this.schemaMetadata, async () => {
            open = false;
            if (tx) {
                const t = await tx;
    ...
    async createTx(from, to) {
        ...
        try {
            if (!this.disableAutoHeightUpdate)
