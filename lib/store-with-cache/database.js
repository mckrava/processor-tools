...
exports.FullTypeormDatabase = exports.TypeormDatabaseWithCache = void 0;
...
class BaseDatabase {
    ...
    async updateHeight(em, from, to) {
        return em.query(`UPDATE ${this.statusSchema}.status SET height = $2 WHERE id = 0 AND height < $1`, [from, to]).then((result) => {
 ...
 */
class TypeormDatabaseWithCache extends BaseDatabase {
    constructor() {
        super();
        this.schemaMetadata = (0, schemaMetadata_1.getSchemaMetadata)();
        this.cacheStorage = store_1.CacheStorage.getInstance();
        console.log('TypeormDatabaseWithCache >>> constructor => ', this.schemaMetadata);
    }
    async runTransaction(from, to, cb) {
        let tx;
        let open = true;
        console.log('TypeormDatabaseWithCache >>> runTransaction');
