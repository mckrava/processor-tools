...
class CacheStorage {
    constructor() {
        ...
        this.cacheMarker = (new Date().getTime() * Math.random()).toString(36).slice(2, 8);
    }
    static getInstance() {
...
class StoreWithCache {
    constructor(em, cacheStorage, schemaMetadata) {
        ...
        // };
        this.storeMarker = '';
        this.storeMarker = (new Date().getTime() * Math.random()).toString(36).slice(2, 8);
    }
    /**
    ...
    async load() {
        ...
        console.log('load storeMarker >>> - ', this.storeMarker);
        console.log('load cacheMarker >>> - ', this.cacheStorage.cacheMarker);
    }
    /**
    ...
    async cacheFlush() {
        ...
        console.log('cacheFlush storeMarker >>> - ', this.storeMarker);
        console.log('cacheFlush cacheMarker >>> - ', this.cacheStorage.cacheMarker);
    }
    async _flushAll() {
    ...
    async _flushByClass(entityConstructor) {
        if (this.cacheStorage.entitiesForFlush.has(entityConstructor)) {
            ...
            const listForSave = [...(this.cacheStorage.entities.get(entityConstructor) || new Map()).values()].filter(entity => forFlush.has(entity.id));
        ...
        if (!this.cacheStorage.deferredRemoveList.has(entityConstructor))
            return;
        await this.remove(entityConstructor, [...(this.cacheStorage.deferredRemoveList.get(entityConstructor) || new Set()).values()]);
    ...
    async remove(e, id) {
        ...
        const enClass = 'id' in singleEnOrClass ? Object.getPrototypeOf(singleEnOrClass).constructor : singleEnOrClass;
        //@ts-ignore
    ...
    find(entityClass, options) {
        return this._processFetch(entityClass, () => this.em().then(em => em.find(entityClass, options)));
    }
    findBy(entityClass, where) {
        return this._processFetch(entityClass, () => this.em().then(em => em.findBy(entityClass, where)));
    }
