...
describe('Store Deferred Methods :: Save/Update', function () {
    ...
    describe('save/update cyclic relations', function () {
        ...
        it('save all possible entity types at once', async function () {
            ...
            const acc = new model_1.Account({ id: '2' });
            // @ts-ignore
            const space = new model_1.Space({ id: '2', createdByAccount: { id: '2' } });
            // @ts-ignore
            acc.profileSpace = { id: '2' };
            // @ts-ignore
            const post = new model_1.Post({ id: '3', createdByAccount: { id: '2' } });
            await store.deferredUpsert(acc).deferredUpsert(post).deferredUpsert(space).flush();
            const respPost = await store.find(model_1.Post, { where: { createdByAccount: { profileSpace: { id: '2' } } } });
            const respAcc = await store.find(model_1.Account, { where: { id: '2' } });
            console.dir(respPost, { depth: null });
            console.dir(respAcc, { depth: null });
        });
    });
});
