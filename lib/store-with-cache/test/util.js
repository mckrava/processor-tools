...
exports.createSaveRelatedEntities = exports.generateListOfItems = exports.getItemIds = exports.getItems = exports.createStore = exports.getEntityManager = exports.useDatabase = exports.databaseDelete = exports.databaseInit = exports.db_config = void 0;
...
function createStore() {
    ...
    return new store_1.Store(getEntityManager, cacheStorage, 1000, schemaMetadata);
}
...
async function createSaveRelatedEntities(store, withFlush = true) {
    const account = new model_1.Account({ id: '1' });
    const space = new model_1.Space({ id: '1', createdByAccount: account });
    account.profileSpace = space;
    const posts = [
        new model_1.Post({ id: '1', createdByAccount: account }),
        new model_1.Post({ id: '2', createdByAccount: account, space })
    ];
    store.deferredUpsert(posts);
    const comment = new model_1.Post({
        id: '2-1',
        createdByAccount: account,
        space,
        parentPost: await store.get(model_1.Post, '2', false)
    });
    if (withFlush)
        await store.deferredUpsert(account).deferredUpsert(space).deferredUpsert(comment).flush();
}
exports.createSaveRelatedEntities = createSaveRelatedEntities;
