...
describe('Store Deferred Methods :: Save/Update', function () {
  ...
  describe('save/update cyclic relations', function () {
    ...
    it('save all possible entity types at once', async function () {
      ...
      const acc = new Account({ id: '2' });
      // @ts-ignore
      const space = new Space({ id: '2', createdByAccount: { id: '2' } });
      // @ts-ignore
      acc.profileSpace = { id: '2' };
      // @ts-ignore
      const post = new Post({ id: '3', createdByAccount: { id: '2' } });

      await store.deferredUpsert(acc).deferredUpsert(post).deferredUpsert(space).flush();

      const respPost = await store.find(Post, { where: { createdByAccount: { profileSpace: { id: '2' } } } });
      const respAcc = await store.find(Account, { where: { id: '2' } });
